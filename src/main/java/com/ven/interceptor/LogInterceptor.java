...
public class LogInterceptor implements HandlerInterceptor {
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		if(request.getRequestURI().equals("/captcha") || request.getRequestURI().equals("/") || request.getRequestURI().equals("/site/login")){
			return;
		}
		...
		Log log = new Log(request.getMethod(),request.getRequestURI(),getUrl(request), userAge ,parameters.toString(), userId, account, ip);
		logService.save(log);
	}
